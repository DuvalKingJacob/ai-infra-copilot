...
> This walkthrough is workspace-based. Stacks and Drift Detection are separate features. Drift Detection runs against standard workspaces. Stacks does not currently have native Drift Detection.
...
`tfctl` is a CLI for the HCP Terraform API that gives operators read-only run and workspace context without requiring console access.
...
The local drift report classifies drift by resource sensitivity and security-relevant attribute changes. It is a transparent stand-in for triage, not an autonomous remediation engine.
...
```text
Use init -backend=false here because this step validates configuration structure, not remote state. Production drift detection reads against HCP Terraform or Terraform Enterprise workspace state.
```
...
| `make drift-report` | prioritized drift triage report is generated by resource sensitivity and security-relevant attribute changes |
